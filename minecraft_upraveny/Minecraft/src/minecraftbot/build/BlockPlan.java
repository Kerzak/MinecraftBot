...
public class BlockPlan implements ILocated
{
    ...
    /**
     * Create copy of a plan, with different coordinates.
     * @param plan original plan.
     * @param x X coordinate offset.
     * @param y Y coordinate offset.
     * @param z Z coordinate offset. 
     */
    public BlockPlan(BlockPlan plan, int x, int y, int z)
    {
        this.id = plan.id;
        this.x = plan.x+x;
        if(plan.y+y<0)
            throw new InvalidParameterException("Y cannot be lesser than 0: y="+plan.y+y);
        if(y>255)
            throw new InvalidParameterException("Y cannot be greater than 255: y="+plan.y+y);
        this.y = (byte)(y+plan.y);
        this.z = plan.z+z;
    }
